...
class LocationService {
    ...
    async getLocation(event, context) {
        ...
        try {
            ...
            //respuestas de cordenadas utilizando libreria de triangulacion
            this.calculationLibrary(distance1, distance2, distance3);
    ...
    calculationLibrary(s1, s2, s3) {
        trilateration.addBeacon(0, trilateration.vector(x1, y1));
        trilateration.addBeacon(1, trilateration.vector(x2, y2));
        trilateration.addBeacon(2, trilateration.vector(x3, y3));

        trilateration.setDistance(0, s1);
        trilateration.setDistance(1, s2);
        trilateration.setDistance(2, s3);

        const pos = trilateration.calculatePosition();
        console.log("Result for library: "+"X: " + pos.x + "; Y: " + pos.y);
    }

    /* average(X, Y, distance1, distance2, distance3) {
        let d1 = Math.sqrt(((X-x1)*(X-x1)) + ((Y-y1)*(Y-y1)));
        let d2 = Math.sqrt(((X-x2)*(X-x2)) + ((Y-y2)*(Y-y2)));
        let d3 = Math.sqrt(((X-x3)*(X-x3)) + ((Y-y3)*(Y-y3)));
